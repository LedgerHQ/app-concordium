...
> **P2** must be `0x00` for both INIT and CONT frames; any other value is rejected with
...
| `"mint"`           | `amount` (tag 4), `recipient` (tag 40307)[, `memo`] | Mint                   |
| `"burn"`           | `amount` (tag 4), `target` (tag 40307)[, `memo`]   | Burn                   |
...
- **recipient / target** — CBOR tag 40307 wrapping a 32-byte byte string (raw account address).
...
| -------- | -------------------------- | -------------------------------------------------------------------------------------- |
| `0x6B00` | `SWO_WRONG_P1_P2`          | P2 ≠ `0x00`                                                                            |
...
| `0x6A80` | `SWO_INCORRECT_DATA`       | Trailing bytes after `cbor_total_length` in the INIT frame                             |
