"""
Tests for INS 0x27 — PLT (Protocol Level Token) blind-signing flow.

The handler is currently in temporary blind-signing mode: it hashes and signs
the transaction without displaying anything to the user (no UI navigation).
All exchanges are therefore synchronous — no exchange_async / navigator calls.
...
The last CONT returns a 64-byte Ed25519 signature in the response data.
...
# CBOR payloads
# Single-frame CBOR: map(1) { "x" => 1 }  — 4 bytes, fits in one CONT frame.
_CBOR_SMALL = bytes.fromhex("a1617801")

# Multi-frame CBOR: byte-string of 300 zero bytes (CBOR overhead: 3 bytes header)
# = 0x59 0x01 0x2C <300 × 0x00>  —  303 bytes, needs two CONT frames (255 + 48).
...
def test_sign_plt_single_cont_frame(backend):
    """INIT + one CONT frame: small CBOR that fits in a single chunk."""
...
def test_sign_plt_exact_cbor_max(backend):
    """CBOR payload of exactly APP_PLT_CBOR_MAX bytes is accepted."""
    client = CommandSender(backend)
    # Build a valid CBOR blob of exactly APP_PLT_CBOR_MAX bytes.
    # Use an indefinite-length byte string to avoid complex length encoding:
    # 0x5f = begin indefinite byte string, single chunk of (MAX-2) bytes, 0xff = break.
    # Chunk header: 0x58 <len-byte> for length <= 255, or 0x59 <2-byte-len>.
    # Easiest: one fixed-length byte string with a 3-byte CBOR header:
    #   0x59 <uint16_be(MAX-3)> <MAX-3 zero bytes>  →  total = MAX bytes.
    inner_len = _APP_PLT_CBOR_MAX - 3
    cbor = bytes([0x59, (inner_len >> 8) & 0xFF, inner_len & 0xFF]) + bytes(inner_len)
    assert len(cbor) == _APP_PLT_CBOR_MAX

    resp = client.sign_plt(_PATH, _HEADER_60, _TOKEN_ID_MIN, cbor)
...
def test_sign_plt_intermediate_cont_returns_no_data(backend):
    ...
    # First chunk (255 bytes) — not the last, so response has no payload.
    first_chunk = _CBOR_LARGE[:255]
    resp = client.sign_plt_cont(first_chunk)
    assert resp.status == StatusWords.SWO_SUCCESS
    assert resp.data == b""

    # Final chunk — carries the signature.
    last_chunk = _CBOR_LARGE[255:]
    resp = client.sign_plt_cont(last_chunk)
    assert resp.status == StatusWords.SWO_SUCCESS
    assert len(resp.data) == 64
...
def test_sign_plt_cont_overflow(backend):
    ...
    # Declare 4 bytes but send 5 in the first CONT.
